...
URL = 'http://127.0.0.1:9001/k/max/{start}/{end}/{code}?k={k}'
...
async def process(file_name, start, end, k=90, min_timetomarket=None):
    ...
    for r in rsts:
        print(r[0])
    cnt = len(rsts)
    print('共{cnt}只股票'.format(cnt=cnt))
...
def main(start, end,  last_days, file_name, min_timetomarket):
    """
    近{last_days}破新高的股票列表
    """
    print('='*50)
    print('近{last_days}天，从{start}至{end} 破新高的股票列表:'.format(**locals()))
    import time
    b = time.time()
